...
The build was tested on the Ubuntu 20.04 LTS.
The recommended hardware on the build host is Intel i7, RAM 32GB, and SSD with
150GB of free space.

You may set up a docker container and build product inside it.
Please follow the [doc/Docker.md](doc/Docker.md). And proceed to
the [Building](#building) section.

You may also manually set build environment on the PC.
In that case, install moulin
```
pip3 install --user git+https://github.com/xen-troops/moulin
```
See [moulin build system](https://moulin.readthedocs.io/en/latest/)
...
The build system will create the build directory `yocto/` for yocto's
...
Turn off the board. Write down the state of the SW1[5..8]. Set SW1[5..8]
to `OFF`. Turn on the board.
...
Turn off the board. Set SW1[5..8] to the state written down earlier.
Turn on the board.

### Flash the image and add test tools
Boot the board using Renesas BSP by TFTP/NFS.

Put the full.img file, generated by the build, into the root of the rootfs
used to boot the BSP.
```
...
let's assume its name is `tools/`, with the testing tools into the root of
the BSP's rootfs.
...
After the reboot, stop u-boot autoload and run the image from the eMMC
```
...
You should see the rendered clip on the display and the following logs
in the console
```
# ./SurroundViewDemo_wo_camera 
...
NOTE2: We have used a hacked version of the `isp_rvas_mono_v4h2` with
hard-coded GPIO-10 directly in binary due to a minor hack of the strings.
That was the only option that was successfully working with the camera
for us in DomD.
...
The following log of the `SurroundViewDemo_IMX623_rvas_v4h2` is intended to
show that all OpenCV libraries are in place, and the application starts
